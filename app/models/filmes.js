module.exports = () => {
    this.getAllFilmes = (connection,callback) => {
        connection.query("SELECT fc.idfilmes_copias AS codigo,f.titulo,f.diretor FROM filmes_copias AS fc INNER JOIN filmes AS f ON(f.idfilmes = fc.idfilmes) ORDER BY f.titulo",callback)
    }
    ...
    this.getFilmesID = (id, connection, callback) => {
        connection.query("SELECT fc.idfilmes_copias AS codigo,f.titulo,f.diretor FROM filmes_copias AS fc INNER JOIN filmes AS f ON(f.idfilmes = fc.idfilmes) WHERE fc.idfilmes_copias = "+id, callback)
    }
